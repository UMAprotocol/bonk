...
import { useFinalizeWithdrawMutation } from "../hooks/useFinalizeWithdrawMutation";
import { SLA } from "./sla-table";

export function FinalizeWithdrawModal({
  modalId,
  sla,
  onClose,
}: {
  ...
  const {
    mutateAsync: finalizeWithdraw,
    isLoading,
    isError,
    isSuccess,
    error,
  } = useFinalizeWithdrawMutation();
    ...
    >
      <div className="modal-box">
        <div className="flex flex-row items-center justify-between">
          <h3 className="font-bold text-lg">Finalize Withdrawal</h3>
          <form method="dialog">
        ...
        {isSuccess && (
          <div className="alert alert-success mt-8">
            <span>Withdraw finalized successfully!</span>
          </div>
        )}
        {isError && (
