...
export function useFinalizeWithdrawMutation(opts?: { onSuccess?: () => void }) {
    ...
    mutationFn: async (slashArgs: { stakerId: string }) => {
      ...
      if (address) {
        const { request } = await publicClient.simulateContract({
          ...
          functionName: "finalizeWithdrawStake",
          args: [slashArgs.stakerId as `0x${string}`],
        });
        const finalizeWithdrawStakeTxHash = await walletClient.writeContract(
          request
        );
        await publicClient.waitForTransactionReceipt({
          hash: finalizeWithdrawStakeTxHash,
        });
